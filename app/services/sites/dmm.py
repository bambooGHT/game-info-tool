...
class DMMCrawler(AsyncBaseCrawler[DMMModel]):
    """dmm网站爬虫实现"""
    ...
    async def search(self, query: str, **kwargs) -> Result[List[DMMModel], str]:
        """搜索dmm游戏
        ...
        Returns:
            Result[List[DMMModel], str]: 游戏信息列表
        """
        try:
    ...
    async def get_detail(self, url: str, **kwargs) -> Result[DMMModel, str]:
        """获取游戏详情
    ...
    async def _get_search_html(
        ...
        return await self._make_request(
            f"{self.base_url}/search/?service=pcgame&searchstr={query}",
            method="GET",
        )
